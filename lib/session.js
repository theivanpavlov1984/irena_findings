...
function secret() {
  // Отдельный секрет для подписи сессий. Если он не задан, временно
  // используется ключ Supabase — но тогда его замена разлогинит всех сразу,
  // поэтому SESSION_SECRET лучше задать явно.
  ...
  if (!process.env.SESSION_SECRET && process.env.NODE_ENV === "production") {
    console.warn("SESSION_SECRET не задан — сессии подписаны ключом Supabase");
  }
  return s;
}
